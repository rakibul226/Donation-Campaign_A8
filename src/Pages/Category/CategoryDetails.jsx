...
const CategoryDetails = ({ categoryItem }) => {
  const { product_id, picture, title, price, text_color, description } =
    categoryItem || {};
  ...
  const handleAddToDonation = () => {
    ...
    if (!donatedItems) {
      ...
      const isExist = donatedItems.find(
        (categoryItem) => categoryItem.product_id === product_id
      );
      if (!isExist) {
  ...
  return (
    <div className="px-2 md:px-0 md:mx-14 lg:mx-36">
      <div
        className="hero w-auto "
        style={{ backgroundImage: `url(${picture})`, height: "450px" }}
      ></div>
      <div className="hero-overlay h-24 bg-opacity-50 relative bottom-24">
        <button
          className="px-5 py-1 rounded mt-8 ml-10"
